...
function createMorayClient(opts, cb) {
        ...
        // MANTA-1226
        // It's possible a moray restarted in the middle
        // here, so we need to specifically retry in
        // that instance since there was nothing wrong
        // (most likely) with the putBucket request
        client.once('error', function (err) {
                client.removeAllListeners('connect');

                log.debug(err, 'ring.moray: failed to setup: %s', opts.host);

                var tmp = err.jse_cause || err;
                if (/^ConnectionClosedError$/.test(tmp.name)) {
                        var d = opts.delay || 1000;
                        opts.delay = Math.min(d * 2, 10000);
                        setTimeout(createMorayClient.bind(null, opts, cb), d);
                } else {
                        cb(err);
                }
        });
 ...
 *
 * struct shark {
 *         manta_storage_id <string>,  // mako's storage id
 * }
 *
 * struct object {
